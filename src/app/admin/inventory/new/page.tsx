...
export default function NewInventoryPage() {
  ...
  const validateImageUrl = (i: number, val: string) => {
    if (val && !val.startsWith('http')) {
      setImageErrors((prev) => ({ ...prev, [i]: 'Must start with http' }))
    } else {
      setImageErrors((prev) => {
        const next = { ...prev }
        delete next[i]
  ...
  return (
    <div style={{ maxWidth: '800px' }}>
      <h1 style={{ fontSize: '24px', fontWeight: 700, marginBottom: '24px' }}>Add New Inventory</h1>
        ...
        >
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '14px', marginBottom: '16px' }}>
            ...
            <div>
              ...
              <input name="make" value={form.make} onChange={handleChange} style={inputStyle} />
            </div>
            <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px', marginBottom: '16px' }}>
            ...
            <div>
              ...
              <input name="salePrice" type="number" step="0.01" value={form.salePrice} onChange={handleChange} style={inputStyle} />
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px', marginBottom: '16px' }}>
            ...
            <div>
              ...
              <input name="stockNumber" value={form.stockNumber} onChange={handleChange} style={inputStyle} />
            </div>
          </div>
              ...
              <div key={i} style={{ display: 'flex', gap: '8px', marginBottom: '8px', alignItems: 'flex-start' }}>
                <div style={{ flex: 1 }}>
                  <input
                    ...
                    onBlur={(e) => validateImageUrl(i, e.target.value)}
                    placeholder="https://example.com/image.jpg"
                    style={{
                      ...inputStyle,
                      borderColor: imageErrors[i] ? '#EB2E25' : 'rgba(255,255,255,0.12)',
                    }}
                  />
                  {imageErrors[i] && (
                    <div style={{ color: '#EB2E25', fontSize: '11px', marginTop: '4px' }}>{imageErrors[i]}</div>
                  )}
                </div>
                <button
                  type="button"
