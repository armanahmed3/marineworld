...
export default function ServiceRequestsPage() {
  ...
  const statusBadge = (status: string) => {
    const colors: Record<string, string> = { pending: '#f8941e', in_progress: '#3b82f6', completed: '#22c55e', cancelled: '#EB2E25' }
    return (
  ...
  return (
    <div>
      <h1 style={{ fontSize: '24px', fontWeight: 700, marginBottom: '24px' }}>Service Requests</h1>

      <div style={{ background: '#1a1f35', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.06)', overflow: 'hidden' }}>
        {loading ? (
          <div style={{ padding: '40px', textAlign: 'center', color: '#888' }}>Loading...</div>
        ) : requests.length === 0 ? (
          <div style={{ padding: '40px', textAlign: 'center', color: '#888' }}>No service requests found.</div>
        ) : (
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
              <thead>
                <tr style={{ color: '#888', textTransform: 'uppercase', fontSize: '11px', letterSpacing: '0.5px' }}>
                  ...
                  <th style={{ padding: '12px 16px', textAlign: 'left', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>Service Type</th>
                ...
                {requests.map((r, i) => (
                  <React.Fragment key={r.id}>
                    ...
                    >
                      ...
                      <td style={{ padding: '12px 16px', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>{r.serviceType || '—'}</td>
                    ...
                    {expandedId === r.id && (
                      <tr key={`exp-${r.id}`}>
                        <td colSpan={7} style={{ padding: '16px 20px', background: 'rgba(0,0,0,0.15)', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                          <div style={{ color: '#aaa', fontSize: '13px', marginBottom: '4px' }}>Vehicle Info:</div>
                          <div style={{ color: '#fff', fontSize: '14px', marginBottom: '10px' }}>{r.vehicleInfo || 'Not provided'}</div>
