...
export default function NewInventory() {
  ...
  useEffect(() => {
    fetch('/api/inventory?condition=new')
      .then((r) => r.json())
      .then((data) => {
  ...
  return (
    <div style={{ padding: '60px 20px', maxWidth: '1200px', margin: '0 auto' }}>
      <h1 style={{ fontSize: '42px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '8px' }}>New Inventory</h1>
      <p style={{ color: '#999', fontSize: '16px', marginBottom: '30px' }}>Brand new boats ready for the water</p>
      ...
      {loading ? (
        <div style={{ textAlign: 'center', padding: '60px 0', color: '#999' }}>Loading new inventory...</div>
      ) : filtered.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '60px 0', color: '#999' }}>
          <p style={{ fontSize: '18px' }}>No new inventory items currently available.</p>
          <p>Please check back soon or contact us for upcoming arrivals.</p>
        </div>
      ) : (
          ...
          {filtered.map((item) => (
            <div key={item.id} style={{ background: '#0B1325', border: '1px solid #1a2040', borderRadius: '8px', overflow: 'hidden', transition: 'transform 0.3s, border-color 0.3s' }}
              ...
              <div style={{ padding: '20px' }}>
                <h3 style={{ fontSize: '18px', fontWeight: 700, margin: '0 0 4px', lineHeight: 1.2 }}>{item.title || `${item.year} ${item.make} ${item.model}`}</h3>
