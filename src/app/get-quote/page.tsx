...
export default function GetQuote() {
  const [form, setForm] = useState({ firstName: '', lastName: '', email: '', phone: '', inventoryInterest: '', message: '' })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setStatus('success')
        setForm({ firstName: '', lastName: '', email: '', phone: '', inventoryInterest: '', message: '' })
      } else {
        setStatus('error')
      }
    } catch {
  ...
  return (
    <div style={{ padding: '60px 20px', maxWidth: '700px', margin: '0 auto' }}>
      <h1 style={{ fontSize: '42px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '8px' }}>Get a Quote</h1>
      <p style={{ color: '#999', fontSize: '16px', marginBottom: '30px' }}>Interested in a boat? Request a quote and our team will get back to you.</p>

      {status === 'success' ? (
        <div style={{ padding: '40px', textAlign: 'center', background: '#0B1325', borderRadius: '8px', border: '1px solid #1a2040' }}>
          <h2 style={{ color: '#EB2E25', fontSize: '24px', marginBottom: '12px' }}>Quote Request Submitted!</h2>
          <p style={{ color: '#ccc' }}>Thank you for your interest. A member of our sales team will contact you shortly with a personalized quote.</p>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          {status === 'error' && <p style={{ color: '#EB2E25', fontSize: '14px' }}>An error occurred. Please try again or call us at 903.705.0804.</p>}
        </form>
      )}
    </div>
  )
